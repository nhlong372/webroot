<?php
    ...
    if($id_danhmuc){ $where_tmp .= " and id_brand=" . $id_danhmuc; }
...
<?php  if($result_records) {
    ...
    switch($type_select) {
            ...
            case 'phu-kien':
                if (QUICKVIEW == false) {                 
                    echo '<div class="run-slick run-list">'.$func->GetProducts($result_records, '',false,'270x270x2').'</div>';
                    break;
                } else {
                    echo '<div class="run-slick run-list">' . $func->GetProducts2($result_records, '',false,'270x270x2') . '</div>';
                    break;
                }      
        } ?>
