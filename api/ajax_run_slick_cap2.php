...
<?php if($result_records) {
    ...
    switch($type_select) {
            ...
            case 'phu-kien':
                if(QUICKVIEW == false) {                 
                    echo '<div class="run-slick run-cat'.$id_list.$id_danhmuc.'">'.$func->GetProducts($result_records, '',false,'270x270x2').'</div>';
                    break;  
                } else {  
                    echo '<div class="run-slick run-cat'.$id_list.$id_danhmuc.'">'.$func->GetProducts2($result_records, '',false,'270x270x2').'</div>';
                    break;
                }       
        } ?>
